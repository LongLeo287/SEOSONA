    ...
    items: [
      {
        ...
        href: "/dich-vu-seo/",
        icon: <Search className="h-6 w-6 text-[#3BA6F1]" />
      },
      {
        ...
        icon: <BarChart className="h-6 w-6 text-[#3BA6F1]" />
      },
      {
        title: "Audit SEO",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: <Share2 className="h-6 w-6 text-[#3BA6F1]" />
      },
      {
        title: "YouTube Ads",
            ...
            {services.map((group, groupIndex) => (
              <div key={groupIndex} className="flex flex-col">
                <h2 className="text-2xl font-black text-[#04091A] mb-8 pb-4 border-b border-slate-100 flex items-center gap-3">
                  <div className="h-6 w-1 bg-[#3BA6F1] rounded-full"></div>
                  {group.category}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {group.items.map((item, index) => (
                    <Link href={item.href} key={index} className="group relative flex flex-col p-8 rounded-3xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-[#3BA6F1]/30 hover:shadow-[0_12px_40px_rgba(59,166,241,0.12)] transition-all duration-300 hover:-translate-y-1 overflow-hidden">
                      ...
                      <h3 className="text-[20px] font-bold text-[#04091A] mb-3 group-hover:text-[#3BA6F1] transition-colors">{item.title}</h3>
                      <p className="text-[15px] text-slate-500 font-medium leading-relaxed mb-6 flex-1">
                        {item.description}
                      </p>
                      <div className="flex items-center text-[14px] font-bold text-[#3BA6F1]">
                        Tìm hiểu thêm
