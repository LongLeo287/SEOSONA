...
const solutions = [
  {
    ...
    icon: <Bot className="h-6 w-6 text-[#3BA6F1]" />,
    highlight: true
  },
  {
    ...
    icon: <MessageCircle className="h-6 w-6 text-[#3BA6F1]" />,
    highlight: false
  },
  {
    ...
    icon: <BarChart2 className="h-6 w-6 text-[#3BA6F1]" />,
    highlight: false
  },
  {
    ...
    icon: <Zap className="h-6 w-6 text-[#3BA6F1]" />,
    highlight: false
  }
];
        ...
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {solutions.map((item, index) => (
              <div key={index} className={`group relative flex flex-col p-8 sm:p-10 rounded-[32px] bg-white border ${item.highlight ? 'border-[#3BA6F1] shadow-[0_12px_40px_rgba(59,166,241,0.12)]' : 'border-slate-200/80 shadow-sm'} hover:-translate-y-1 hover:shadow-xl transition-all duration-300 overflow-hidden`}>
                {item.highlight && (
                  <div className="absolute top-0 right-0">
                    <div className="bg-[#3BA6F1] text-white text-[12px] font-bold px-4 py-1.5 rounded-bl-2xl">
                      Featured
                    </div>
                  </div>
                )}
                ...
                <h3 className="text-[24px] font-black text-[#04091A] mb-4 group-hover:text-[#3BA6F1] transition-colors">{item.title}</h3>
                <p className="text-[16px] text-slate-500 font-medium leading-relaxed mb-8 flex-1">
                  {item.description}
                </p>
                <Link href={item.href} className="inline-flex items-center text-[15px] font-bold text-[#3BA6F1] bg-[#3BA6F1]/10 px-5 py-3 rounded-full w-fit hover:bg-[#3BA6F1] hover:text-white transition-colors">
                  Khám phá giải pháp
