          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative flex flex-col rounded-[2.5rem] p-6 sm:p-8 ${
                plan.isPopular
                  ? "bg-white text-slate-950 shadow-2xl ring-2 ring-blue-600 lg:scale-105 z-10"
                  : "bg-white text-slate-950 shadow-lg ring-1 ring-slate-200/50"
              }`}
            >
              {plan.isPopular && (
                <div className="absolute -top-5 left-0 right-0 mx-auto w-fit rounded-full bg-blue-600 px-5 py-1.5 text-sm font-bold text-white shadow-md">
              ...
              <Link
                ...
                className={`mt-auto block rounded-full px-6 py-4 text-center text-[15px] font-bold transition-all duration-300 ${
                  plan.isPopular
