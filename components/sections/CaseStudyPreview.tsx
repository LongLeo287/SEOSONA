...
const cases = [
  {
    ...
    detail: "Tối ưu cấu trúc danh mục, content hub và internal link theo hành trình mua hàng.",
  ...
  },
  {
    ...
    detail: "Kết hợp SEO technical, landing page, tracking và nội dung chuyển đổi.",
    ...
    <section className="relative overflow-hidden bg-slate-50 py-14 sm:py-16 lg:py-20">
      ...
      <div className="container relative">
        ...
        <div className="grid gap-5 lg:grid-cols-[.82fr_1.18fr]">
          <div className="rounded-[32px] border border-blue-100 bg-gradient-to-br from-blue-600 to-cyan-500 p-7 text-white shadow-2xl shadow-blue-900/15 sm:p-8">
            <div className="grid h-14 w-14 place-items-center rounded-2xl bg-white/15 text-white backdrop-blur">
              <BarChart3 size={28} />
            </div>
            <h3 className="mt-6 text-3xl font-black tracking-tight sm:text-4xl">Không chỉ tăng traffic, mà tăng đúng cơ hội kinh doanh</h3>
            <p className="mt-4 leading-8 text-blue-50">Case study của SEOSONA tập trung vào các chỉ số có ý nghĩa: khả năng index, tăng trưởng truy cập chất lượng, số lượng cụm nội dung và chuyển đổi thành lead.</p>
            <div className="mt-6 grid gap-3">
              {["Audit trước khi lập roadmap", "Đo lường bằng GSC/GA4", "Tối ưu theo mục tiêu kinh doanh"].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl bg-white/12 p-3 font-bold text-white">
                  <CheckCircle2 size={19} />
                  <span>{item}</span>
                </div>
              ))}
            </div>
            <Link href="/case-study/" className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 font-black text-blue-700 shadow-lg shadow-blue-950/10 transition hover:-translate-y-0.5 hover:bg-blue-50">Xem tất cả case study <ArrowRight size={18} /></Link>
          </div>

          <div className="grid gap-5 md:grid-cols-3 lg:grid-cols-1">
            {cases.map((item) => (
              <Link href="/case-study/" key={item.industry} className="group relative overflow-hidden rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl lg:grid lg:grid-cols-[160px_1fr_auto] lg:items-center lg:gap-5">
